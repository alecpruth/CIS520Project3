...
bool alloc_big_stack(uint8_t *where)
{
  uint8_t *kpage;
  bool success = false;
  ...
  kpage = palloc_get_page (PAL_USER | PAL_ZERO);
    if (kpage != NULL) 
    {
      success = install_page (where, kpage, true);
      if (!success) {
        printf("Failed!\n");
...
setup_stack (const char *cmd_line, void **esp) 
{
 ...
 /*
  unsigned cnt = 1;
  void * where = ((uint8_t *) PHYS_BASE) - 2*PGSIZE;
  
  for(; cnt ; cnt--, where -= PGSIZE) {
      if( alloc_big_stack(where) == true ) 
        return ok;
  }
*/
return ok;
